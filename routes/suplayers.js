...
router.get("/", (req, res) => {
  const sql = "SELECT * FROM Suplayer";
  db.all(sql, [], (err, rows) => {
    if (err) {
...
router.get("/:id", (req, res) => {
  const { id } = req.params;
  const sql = "SELECT * FROM Suplayer WHERE id = ?";
  const params = [id];
...
router.post("/", (req, res) => {
  ...
  const sql = `INSERT INTO Suplayer (name, phone, address, company, add_date) 
                VALUES (?, ?, ?, ?, ?)`;
...
router.delete("/:id", (req, res) => {
  const { id } = req.params;
  const sql = `DELETE FROM Suplayer WHERE id = ?`;
  const params = [id];
  db.run(sql, params, function (err) {
    if (err) {
      res.status(400).json({ error: err.message });
